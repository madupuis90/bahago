...
- `web/static/` — static assets (JS); `web/static/styles.css` is a **generated build artifact** — never read or edit it directly
- `web/css/` — CSS source files (edit these); run `task css:build` after any change to regenerate `styles.css`
...
- **Me → Claude Design**: anything I commit (`web/css/` source files, `design/components.md`, `design/naming.md`) is readable by Claude Design at the start of its next session. Note: `web/static/styles.css` is gitignored (generated artifact) — Claude Design reads the source files instead.
...
3. Implement CSS in the correct `web/css/` source file for that feature area (see CSS source file map below); implement markup in the feature handler package. Run `task css:build` after editing CSS.
...
CSS is split into source files under `web/css/`. **Never read or edit `web/static/styles.css`** — it is generated by `task css:build` and is gitignored.

When working on CSS, read only the file(s) relevant to the feature being changed:

| File | Feature area |
|------|-------------|
| `web/css/00-reset.css` | CSS reset |
| `web/css/01-tokens.css` | Design tokens (`:root` custom properties) |
| `web/css/02-base.css` | Base element styles (`html`, `body`, `a`, inputs) |
| `web/css/10-home-shell.css` | Home shell: top-nav, content-area, side-nav |
| `web/css/20-shared.css` | Shared components: `.panel`, `.btn`, `.form-fields`, `.alert-*`, etc. |
| `web/css/30-auth.css` | Auth pages (`.auth-card`) |
| `web/css/31-home.css` | Home page content |
| `web/css/40-kingdom-chrome.css` | Kingdom chrome: topbar, bottom-nav, parchment helpers, `.nav-stone` |
| `web/css/41-kingdom-overview.css` | Kingdom overview: `.page-header`, `.overview-grid`, `.chronicle`, `.demesne` |
| `web/css/42-allocation.css` | Allocation: `.allocation-*` |
| `web/css/43-buildings.css` | Buildings: `.buildings-*`, `.building-*` |
| `web/css/44-units.css` | Units: `.units-*`, `.unit-*` |
| `web/css/45-world-map.css` | World map: `.map-*` |
| `web/css/46-army.css` | Army: `.army-*` |
| `web/css/47-flipcard.css` | Home/about flip card: `.flip-*` |
| `web/css/48-messages.css` | Messages: `.messages-*`, `.message-*` |
| `web/css/49-guild.css` | Guild: `.guild-*`, `.guilds-*` |
| `web/css/50-prayers.css` | Prayers: `.prayer-*`, `.prayers-*` |
| `web/css/99-utilities.css` | Utility overrides: `.text-positive`, `.text-negative` |

After editing any source file, run `task css:build` to regenerate `web/static/styles.css`. During `task dev`, air handles this automatically on every CSS file change.

New feature sections go in a new numbered file between the last feature file and `99-utilities.css`. Styles shared across 2+ features go in `20-shared.css`.
...
- `task dev` — development with live reload (automatically runs `task css:build` first)
...
- `task css:build` — concatenate `web/css/*.css` into `web/static/styles.css`; run after editing any CSS source file
