		...
		Div(Class("slider-controls v-badge"),
			...
			Div(Class("slider-wrap"),
				Div(Class("slider-track"),
					Div(Class("slider-fill"), ds.Style("width", fmt.Sprintf("'calc('+$%s/100+' * (100%% - 40px))'", key))),
					Div(Class("slider-ticks")),
					Div(Class("slider-thumb"), ds.Style("left", fmt.Sprintf("'calc(20px + '+$%s/100+' * (100%% - 40px))'", key)), ds.Text(fmt.Sprintf("$%s", key))),
				),
