...
How the pi coding-agent tooling is configured inside the dev container. This is
housekeeping, not game-domain guidance — domain conventions live in `AGENTS.md`.
...
Project skills live in `.pi/skills/` and are discovered by pi automatically once
the project is trusted: `grill-me`, `grill-with-docs`, `handoff`, `pickup`,
`code-review`. Invoke with `/skill:<name>`.
